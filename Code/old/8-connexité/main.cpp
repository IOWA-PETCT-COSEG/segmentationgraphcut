...
int main( int argc, char *argv[]  )
{
	...
	for (int i=0; i<nx; i++)
	{
		for (int j=0; j<ny; j++)
		{
			...
			draw_edges_image_data(G, image, i, j, i, j+1, nx, ny, lambda, sigma,1);	//Down
			draw_edges_image_data(G, image, i, j, i+1, j, nx, ny, lambda, sigma,1);	//Right
			draw_edges_image_data(G, image, i, j, i+1, j-1, nx, ny, lambda, sigma, sqrt(2.));	//Top right
			draw_edges_image_data(G, image, i, j, i+1, j+1, nx, ny, lambda, sigma, sqrt(2.));	//Down right
